...
TypeDefBlock  : TYPE TypeDefList ENDTYPE          {  printf("typedef Completed\n");
                                                       }            
              ;         
TypeDefList   : TypeDefList TypeDef
              | TypeDef
              ;
TypeDef       : Typedefh  Typedefb            {}
              ;
              
Typedefh      :ID                             { strcpy(id1,$1->name);
                                                TT4=TLookup(id1);
                                                if(TT4==NULL){
                                                  TInstall($1->name,0,NULL);}}
              ;
Typedefb      :'{' FieldDeclList '}'         { TT3=TLookup(id1); 
                                              TT3->fields=FHead;
                                              TT3->size=cf;
                                              cf=0;
                                              FHead=NULL;
...
FieldDecl    : Type1 ID ';'               { FInstall(TT2,$2->name);
                                            }
             ;
...
stmt  : ID '=' E ';'                       { //printf("in id=e\n");
        ...
        |FREE '(' ID ')' ';'       {   $$=makenode(TLookup("void"),61," ",0,$3,NULL,NULL);
                                       L1=Llookup($3->name);
                                                if(L1==NULL){
						A2=A1;
					       while(A2!=NULL){
...
E  :  E '+' E                { TT2=TLookup("bool");
     ...
     |INITIALIZE '(' ')'  {$$=makenode(TLookup("int"),56,"",0,NULL,NULL,NULL);
                          // printf("initialize\n");
                           }        
     |Field            {   $$=makenode(TLookup("void"),55," ",0,NULL,NULL,NULL); 
                          flisthead=flist2;
                           strcpy(nm,flist2->name);
                           strcpy($$->name,flisthead->name);
                           L1=Llookup(nm);
                         temp=NULL;
		       if(L1==NULL){
		                  A2=A1;
