...
var : ID                      {if(i==1){
                                  // printf("%d\n",i);
                                   p=(int *)malloc(sizeof(int));Ghead=createentry($1->name,30,sizeof(int),p,NULL);
                                   // INT - 30
                                   
                                   }
                               else{
                                   q=(int*)malloc(sizeof(int));createentry($1->name,40,sizeof(int),q,NULL);
                                   //BOOL -40
                                   }
                                   }
     |ID '[' NUM ']'          {if(i==1){
                                   p=(int *)malloc($3->value*16);createentry($1->name,31,$3->value,p,NULL);
                                   //INT_ARR - 31
                                 
                                   }
                               else{
                                   q=(int*)malloc($3->value* 16);createentry($1->name,41,$3->value,q,NULL);
                                   //BOOL_ARR - 41
                                   }
                              } 
     ;
